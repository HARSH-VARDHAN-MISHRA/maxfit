...
const TapCategory = () => {
    ...
    const hadndleDelete = async (id)=>{
        try {
            const res = await axios.delete(`http://localhost:7000/api/v1/delete-tap-category/${id}`);
    ...
    <>
        ...
        <section className="breadCmb">
                <div>
                    ...
                    <ul>
                        <li><a href="/">Home / </a></li>
                        <li>Our tap Categories</li>
                    </ul>
                </div>
                <div className="btn1">
                    <Link to={'/create-tap-category'} >Create Tap Category </Link>
                </div>
            </section>
                        ...
                        <table className="table table-bordered">
                            ...
                            <tbody>
                                {categories && categories.reverse().map((item,index)=>(
                                    <tr key={index}>
                                        <td>{index+1}</td>
                                        <td><img src={item.categoryImage} alt="Tap-Category-image" /></td>
                                        <td>{item.categoryName}</td>
                                        <td className='upd-btns'>
                                            <Link to={`/update-tap-category/${item._id}`} className='upd-btns update'><i className="fa-solid fa-pen-to-square"></i></Link>
